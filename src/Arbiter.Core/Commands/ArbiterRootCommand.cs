...
namespace Arbiter.Core
{
    public class ArbiterRootCommand : RootCommand
    {
        ...
        public ArbiterRootCommand(IMSBuildSolutionAnalyzer analyzer, IConsole console, ChildCommands childCommands) : base("Only run the tests you need based on the changes between builds")
        {
            ...
            Handler = CommandHandler.Create<FileInfo, string, bool, CancellationToken>(ExecuteHandler);
        ...
        public async Task<int> ExecuteHandler(FileInfo solution, string fromCommit, bool verbose, CancellationToken token)
        {
            Globals.Verbose = verbose;

            await _childCommands.Analyze.ExecuteHandler(solution, fromCommit, verbose, token, true);
